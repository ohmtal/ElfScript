...
//-----------------------------------------------------------------------------
void BasicGrid::init(ConsoleVector lArea, F32 lSquareSize)
{
	mInitDone = false;
	SAFE_DELETE_ARRAY(mNodes);
	mArea    = lArea; 
	...
	mNodesX = ElfMath::mCeil(mArea.points[2] / mSquareSize);
	mNodesY = ElfMath::mCeil(mArea.points[3] / mSquareSize);
	mNodeCount = mNodesX * mNodesY;

	if (mNodeCount <= 0)
	{
		Con::errorf("BasicGrid: Invalid area!");
		return;
	}
	...
	for (i=0; i<mNodesX; i++)
		for (j=0; j<mNodesY; j++)
		{
			...
			F32 lz = 0.f;
			ConsoleVector lPos = { mArea.points[0] + (F32)i * mSquareSize, mArea.points[1] + (F32)j * mSquareSize };
			lNode->setPos(lPos);
			n++;
...
S32 BasicGrid::getNodeIndex(F32 x, F32 y)
{
	...
	if (!ElfMath::pointInRect({x,y}, mArea)) return -1;

	//  Calculate the "relative" grid position
	F32 fx = (x - mArea.points[0]) / mSquareSize;
	F32 fy = (y - mArea.points[1]) / mSquareSize;

	// 3. Convert to integer and CLAMP
	// This ensures that even if x is exactly at the edge, it stays within valid indices
	S32 xNode = ElfMath::mClamp(static_cast<S32>(ElfMath::mFloor(fx)), 0, mNodesX - 1);
	S32 yNode = ElfMath::mClamp(static_cast<S32>(ElfMath::mFloor(fy)), 0, mNodesY - 1);
...
//-----------------------------------------------------------------------------------------------------
bool BasicGrid::getNodesByRect(const ConsoleVector &lRect, std::vector<S32> &lList, bool lCanOverlap)
{
	if (!isInitialized()) return false;
	if (!ElfMath::isValidRect(lRect))
		return false;

	// Initial calculation using floor to ensure we get the correct starting cell
	S32 xB = static_cast<S32>(ElfMath::mFloor((lRect.points[0] - mArea.points[0]) / mSquareSize));
	S32 yB = static_cast<S32>(ElfMath::mFloor((lRect.points[1] - mArea.points[1]) / mSquareSize));

	S32 xE = static_cast<S32>(ElfMath::mFloor((lRect.points[0] + lRect.points[2] - mArea.points[0]) / mSquareSize));
	S32 yE = static_cast<S32>(ElfMath::mFloor((lRect.points[1] + lRect.points[3] - mArea.points[1]) / mSquareSize));
...
bool BasicGrid::checkLineOfSight(ConsoleVector start, ConsoleVector end) {
	if (!isInitialized()) return false;

	F32 dist =  ElfMath::Vec2Dist(start,end);
	if (dist == 0.f) return true; //???
	ConsoleVector dir = ElfMath::Vec4Div( ElfMath::Vec4Sub(end, start) , dist);
	F32 stepSize = mSquareSize / 2.0f;

	for (F32 d = 0.0f; d < dist; d += stepSize) {
		ConsoleVector checkPos = ElfMath::Vec4Add(start, ElfMath::Vec4Mul(dir, d));
		BasicGridNode* node = findNode(checkPos.points[0], checkPos.points[1]);
...
std::vector<ConsoleVector> BasicGrid::getPath(ConsoleVector start, ConsoleVector end, bool smoothPath)
{

	std::vector<ConsoleVector> path;
	std::vector<BasicGridNode*> replyList;

	if (!isInitialized()) return path;

	BasicGridNode* startNode = findNode(start.points[0], start.points[1]);
	BasicGridNode* goalNode = findNode(end.points[0], end.points[1]);
	...
	if (startNode && goalNode && generatePath(startNode, goalNode, replyList, smoothPath))
	{
		...
		for (auto* node : replyList)
		{
			...
			path.push_back({
				node->getPos().points[0] + halfSize,
						   node->getPos().points[1] + halfSize
			});
		}
	}
	return path;
...
//-----------------------------------------------------------------------------
S32 BasicGrid::getPathCosts(ConsoleVector start, ConsoleVector end)
{
	if (!isInitialized()) return -1;
	S32 startIndex, goalIndex;
	BasicGridNode* startNode = findNode(start.points[0], start.points[1], startIndex);
	BasicGridNode* goalNode = findNode(end.points[0], end.points[1], goalIndex);
...
//-----------------------------------------------------------------------------------------------------
S32 BasicGrid::getNodeToNodeCosts(ConsoleVector from, ConsoleVector to)
{
	if (!isInitialized()) return -1;
	// Ensure table exists
	if (mPathCosts.empty()) {
	...
	S32 startNodeIdx = getNodeIndex(from.points[0], from.points[1]);
	S32 endNodeIdx = getNodeIndex(to.points[0], to.points[1]);
...
F32 BasicGrid::estimateCostToGoal(BasicGridNode* from, BasicGridNode* goal)
{
	...
	return ElfMath::Vec2Length(ElfMath::Vec4Sub(  goal->getPos() , from->getPos()));
}
} //namespace
