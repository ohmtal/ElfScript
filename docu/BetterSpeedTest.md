...
- added GridObject 
- removed uncommon static fieldtypes from fast path (if.. cycles) and rolled it back again - did not speed up - maybe switch from if to case ? 

|Script | run 1 | run 2 | run 3 |
| --- | --- | --- | --- |
|test_localvar.elf |1.32s |1.32s |1.32s |
|test_global.elf |2.38s |2.38s |2.30s |
|test_static.elf |4.12s |4.08s |4.09s |
|test_dynamic.elf |3.60s |3.60s |3.65s |
|test_vector_components.elf |3.89s |3.97s |3.88s |


- added ConsoleVector "Methods" only when new CALLFUNC is enables. It plug in into call method

after speed test:

Script | run 1 | run 2 | run 3 |
| --- | --- | --- | --- |
|test_localvar.elf |1.33s |1.31s |1.31s |
|test_global.elf |2.37s |2.44s |2.40s |
|test_static.elf |3.98s |3.93s |3.95s |
|test_dynamic.elf |3.65s |3.62s |3.70s |
|test_vector_components.elf |3.86s |4.18s |3.83s |
...
Script | run 1 | run 2 | run 3 |
| --- | --- | --- | --- |
|test_localvar.elf |1.33s |1.32s |1.33s |
|test_global.elf |2.42s |2.41s |2.41s |
|test_static.elf |4.06s |4.05s |4.06s |
|test_dynamic.elf |3.62s |3.64s |3.61s |
|test_vector_components.elf |3.83s |3.82s |3.80s |


- added "normal" and injected Lambda run/step to PointStorageObject .. 

also make now always a speedtest to keep a eye on - if I did break something.
Very good back on track.

|Script | run 1 | run 2 | run 3 |
| --- | --- | --- | --- |
|test_localvar.elf |1.35s |1.35s |1.36s |
|test_global.elf |2.48s |2.51s |2.44s |
|test_static.elf |4.02s |4.02s |4.01s |
|test_dynamic.elf |3.63s |3.62s |3.61s |
|test_vector_components.elf |3.83s |3.80s |3.79s |



- Removed cvLambda now using cvPointer with subType cvsLambda:

|Script | run 1 | run 2 | run 3 |
| --- | --- | --- | --- |
|test_localvar.elf |1.38s |1.38s |1.38s |
|test_global.elf |2.46s |2.45s |2.43s |
|test_static.elf |4.39s |4.40s |4.47s |
|test_dynamic.elf |3.67s |3.68s |3.68s |
|test_vector_components.elf |3.98s |3.98s |3.97s |
